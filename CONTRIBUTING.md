...
3. Develop locally:
    ...
    - Install the extra dependencies for tests (`pip install kedro-serving[dev,test]`)
    - Apply your changes
...
1. Check the issues:
    - Ensure all the [release issues](https://github.com/Galileo-Galilei/kedro-serving/milestones) are completed. Eventually move the not addressed yet issues to a further release.
    - Create a [new milestone](https://github.com/Galileo-Galilei/kedro-serving/milestones)
2. Create the release candidate:
    - Go to the [create-release-candidate action](https://github.com/Galileo-Galilei/kedro-serving/actions?query=workflow%3Acreate-release-candidate)
    - Click "Run workflow"
    - Enter the part of the version to bump (one of `<major>.<minor>.<patch>`)
3. If the workflow has run sucessfully:
    - Go to the newly openened PR named "[Release candidate `<version>`](https://github.com/Galileo-Galilei/kedro-serving/pulls)"
...
4. Checkout the [publish workflow](https://github.com/Galileo-Galilei/kedro-serving/actions?query=workflow%3Apublish) to see if:
